...
import { Activity, User, Briefcase, MapPin, Loader2, CheckCircle2, Lock, ChevronLeft, ChevronRight, LogIn, Workflow } from "lucide-react";
...
export default function StationPage() {
  ...
  useEffect(() => {
    ...
    const fetchStation = async () => {
      try {
        ...
        const orderedStations = [...res.data].sort((a, b) => a.id - b.id);
        setStations(orderedStations);

        const current = orderedStations.find((s) => s.id === stationId);
        if (!current) {
  ...
  return (
    <div className="min-h-screen pb-10">
      {/* Premium Header */}
      <div className="bg-gradient-to-br from-blue-200/80 to-slate-200 border-b border-slate-300 pt-12 pb-8 px-6 text-center relative overflow-hidden">
        <div className="absolute top-[-50px] left-1/2 -translate-x-1/2 w-64 h-64 bg-blue-300/40 blur-[80px] rounded-full pointer-events-none" />

        <div className="absolute top-4 left-4 right-4 grid grid-cols-3 items-center z-20">
          <button
            type="button"
            onClick={() => previousStation && router.push(`/station/${previousStation.id}`)}
            disabled={!previousStation}
            className="justify-self-start inline-flex items-center gap-1 px-3 py-2 rounded-lg border border-slate-300 bg-white/90 text-slate-700 hover:bg-white disabled:opacity-40 disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            className="justify-self-center inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-blue-300 bg-blue-100 text-blue-900 hover:bg-blue-200"
          >
          ...
          <button
            type="button"
            onClick={() => nextStation && router.push(`/station/${nextStation.id}`)}
            disabled={!nextStation}
            className="justify-self-end inline-flex items-center gap-1 px-3 py-2 rounded-lg border border-slate-300 bg-white/90 text-slate-700 hover:bg-white disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Next
        ...
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="relative z-10">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-tr from-blue-700 to-slate-900 shadow-lg shadow-blue-700/20 mb-4">
            <MapPin className="w-8 h-8 text-white" />
          </div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight mb-2">{station.name}</h1>
          <div className="inline-flex items-center gap-2 bg-white/90 px-4 py-1.5 rounded-full border border-slate-300">
      ...
      <div className="p-6 max-w-lg mx-auto">
        ...
        >
          <h2 className="text-lg font-bold text-slate-900">Current Operators</h2>
          <span className="text-2xl font-black text-slate-700">{station.employees.length}</span>
        </motion.div>
        ...
        {station.employees.length === 0 ? (
          ...
          <div className="space-y-4">
            {station.employees.map((emp: any, idx: number) => (
              <motion.div 
                key={emp.id}
                initial={{ x: -20, opacity: 0 }}
                animate={{ x: 0, opacity: 1 }}
                transition={{ delay: 0.1 + (idx * 0.1) }}
                className="glass rounded-2xl p-5 border-l-4 border-l-blue-700 relative overflow-hidden"
              >
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mt-1.5">
                      ...
                      <span className="text-xs font-medium text-blue-900 bg-blue-100 px-2 py-0.5 rounded border border-blue-300">
                        {emp.shift}
                      </span>
                    </div>
